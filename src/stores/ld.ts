...
export const useLdStore = defineStore('ld', () => {
  ...
  async function waitForModel(modelId: string) {
    ...
    while (Date.now() < deadline) {
      ...
      if (current?.serving_model_id === modelId && current?.state === 'running') {
        return
      }
      await delay(500)
    }
  ...
  async function loadInitial() {
    await Promise.allSettled([refreshInfo(), refreshModels(), refreshStatus()])
  }
  ...
  function tokenize(prompt: string, signal?: AbortSignal) {
    return api.tokenize(prompt, signal)
  }
